...
import { getInputChangeAction,getStockChangeAction } from './thinkReact/createAction'
...
class Searchbar extends React.Component {
    ...
    handleStockChange(event) {
        const action = getStockChangeAction(event.target.checked);
        store.dispatch(action)
    }
    render() {
        return (
            <div style={{ width: 300, marginTop: 20 }}>
                <form action="">
                    <Input type="text" placeholder="search sth" value={this.props.filterText} onChange={this.handleTextChange} />
                    <p style={{ margin: 0 }}>
